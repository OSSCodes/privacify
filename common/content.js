...
async function pasteEventListenerFunction(event) {
  chrome.storage.local.get(['pasteWarning', 'toastPosition'], function(data){
    const toastPosition = data.toastPosition || 'bottom-right';
    const pasteWarning = data.pasteWarning !== undefined ? data.pasteWarning : true;
    if(pasteWarning){
      showToastMessage(toastPosition, 'SENSITIVE DATA MAY BE PRESENT. PROCEED WITH CAUTION! USE PASTE WITH PRIVACIFY TO AVOID ACCIDENTAL DATA EXPOSURE!');
    }
  });
}
